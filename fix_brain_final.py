...
# Find class LLMAgentBrain start
class_start = content.find('class LLMAgentBrain')
class_body_start = content.find('\n', class_start) + 1

# Helper: find method boundaries within class
def find_method_range(content, class_start, method_name):
    pattern = re.compile(rf'^(    def {method_name}\()', re.MULTILINE)
    m = pattern.search(content[class_start:])
    if not m:
        return None, None
    method_start = class_start + m.start()
    
    # Find next method at same indentation (4 spaces) or class end
    next_pattern = re.compile(r'^    def \w+', re.MULTILINE)
    next_match = next_pattern.search(content[method_start + 1:])
    if next_match:
        method_end = method_start + 1 + next_match.start()
    else:
        method_end = len(content)
    return method_start, method_end

# New method bodies
new_build_criteria = '''    def build_criteria(self, jd_text: str, user_notes: str) -> Dict[str, object]:
...
new_initial_plan = '''    def initial_plan(
...
new_observe_round = '''    def observe_round(
        self,
        plan: SearchPlan,
...
new_decide_fetch = '''    def decide_fetch(
        self,
...
new_review_round = '''    def review_round(
        self,
...
# Replace methods
replacements = [
    ('build_criteria', new_build_criteria),
    ('initial_plan', new_initial_plan),
    ('observe_round', new_observe_round),
    ('decide_fetch', new_decide_fetch),
    ('review_round', new_review_round),
]

for method_name, new_body in replacements:
    start, end = find_method_range(content, class_start, method_name)
