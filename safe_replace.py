...
# Helper to replace text between two markers (exclusive of markers themselves)
def replace_between(content, start_marker, end_marker, new_text):
    ...
    if end == -1:
        print(f'END marker not found: {end_marker[:50]}')
        return content
    return content[:start + len(start_marker)] + new_text + content[end:]

# 1. AGENT_SYSTEM_PROMPT
old_system_start = 'AGENT_SYSTEM_PROMPT = """你是一位资深猎头寻访 Agent。你必须用结构化 JSON 做决策。\n原则：'
old_system_end = '\n只输出 JSON，不要 Markdown。"""'
new_system_body = '''你是一位资深猎头寻访 Agent。你必须用结构化 JSON 做决策。
...
content = replace_between(content, old_system_start, old_system_end, new_system_body)
print('system prompt replaced')

# 2. build_criteria prompt
old_bc_start = '        prompt = """请从 JD 中提取"匹配词与岗位要求"草案，输出 JSON：\n{{\n  "keywords_text": "每行一个关键词，5-12个",\n  "requirements_text": "一段简洁岗位要求描述",\n  "position_filter": "职位栏收口词"\n}}\n\n要求：\n1. 只提取寻访真正需要的关键技能、行业、业务、产品、客户或场景背景。\n2. 不输出评分权重。\n3. 不拆复杂硬性/软性结构。\n4. 不要把 JD 里的所有词都搬出来。\n5. requirements_text 要短、清楚、方便人类编辑确认。\n\n【JD】\n{jd}\n\n【补充说明】\n{notes}\n"""'
old_bc_end = '.format(jd=jd_text or "", notes=user_notes or "")'
new_bc_body = '''请从 JD 中提取"匹配词与岗位要求"草案，输出 JSON：
{{
...
content = replace_between(content, old_bc_start, old_bc_end, new_bc_body)
print('build_criteria prompt replaced')

# 3. build_criteria return logic
...
content = replace_between(content, old_df_start, old_df_end, new_df_body)
print('decide_fetch prompt replaced')

# 7. review_round prompt
old_rr_start = '        prompt = """请复盘本轮猎聘寻访，并决定停止或下一轮搜索，输出 JSON：\n{{\n  "action": "continue/stop",\n  "summary": "复盘结论",\n  "next_plan": {{\n    "query": "下一轮搜索栏",\n    "position_filter": "职位栏",\n    "scope": "全部经历/目前职位",\n    "match_mode": "all/any",\n    "filters": {{}},\n    "intent": "下一轮目的",\n    "expected_signal": [],\n    "risk": "风险",\n    "search_hypothesis_type": "core_background/target_company/transferable_scene",\n    "search_hypothesis_text": "下一轮验证的搜索假设"\n  }},\n  "evidence": {{}}\n}}\n\n如果 should_stop 为 true，必须 action=stop。\n下一轮 query 不要重复 used_queries。\n下一轮只能围绕已确认匹配词与岗位要求组合、放宽或收紧，不要发明新的岗位要求。\n\n【should_stop】{should_stop}\n【stop_reason】{stop_reason}\n【target_met】{target_met}\n【上一轮计划】\n{plan}\n【已用 query】\n{used_queries}\n【匹配结果】\n{matches}\n【噪音】\n{noise}\n【JD】\n{jd}\n"""'
...
new_rr_body = '''请复盘本轮猎聘寻访，并决定停止或下一轮搜索，输出 JSON：
{{
  "action": "continue/stop",
...
content = replace_between(content, old_rr_start, old_rr_end, new_rr_body)
print('review_round prompt replaced')
